				...
				if (!req.user) {
					...
					User.findOne({'email': profile.emails[0].value.toLowerCase()}, function (err, user) {
						...
						if (user) {
							...
							newUser.image = 'https://graph.facebook.com/' + newUser.facebook.id + '/picture?height=350&width=250';
				...
				if (!req.user) {
					...
					User.findOne({'email': params.email.toLowerCase()}, function (err, user) {
						...
						if (user) {
							...
							newUser.vkontakte.image2 = params.photo_big;
							newUser.vkontakte.image3 = profile;
